...
ipcMain.handle('save-logs', async (event) => {
...
const splashHTML = `<body>
  <div class="container">
    ...
    <div style="display: flex; flex-direction: row;">
        <button class="action" id="logs">
          Сохранить логи
        </button>

        <button style="margin-left: 20px" id="close" class="action">
          Закрыть
        </button>

        <button style="margin-left: 20px" id="restart" class="action">
          Перезапустить
        </button>
    </div>
  </div>
  ...
  <script>
    ...
    setTimeout(() => {
      ...
      factEl.style.transform = 'translateY(0) scale(1)';
    }, 500);
  </script>
    ...
    window.addEventListener('DOMContentLoaded', () => {
        ...
        btn.onclick = async () => {
          ...
          if (await ipcRenderer.invoke('save-logs') !== true) {
            ...
            btn.textContent = 'Сохранить логи'
            btn.disabled = false
            return
          }


          document.querySelector('#logs-saved').textContent = 'Логи сохранены'
          btn.textContent = 'Сохранить логи'
          btn.disabled = false
        };
    })
